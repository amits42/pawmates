...
This is a 3-way chat between the pet owner, sitter, and admin support. Feel free to communicate about the booking details, schedule, or any questions!
...
Monitor this conversation for support.`
    ...
    try {
      ...
      const responseMessage = roomExists
        ? `${isAdminOnlyChat ? "Support request" : "Chat message"} sent! Your message for booking #${bookingId} has been sent${
            isAdminOnlyChat ? " to our support team" : ` to ${sitterAlias} and admin support`
          }. (${successCount}/${totalCount} messages delivered)`
        : `${isAdminOnlyChat ? "Support chat" : "3-way chat"} created! WhatsApp chat for booking #${bookingId} has been set up${
            isAdminOnlyChat ? " with our support team" : ` with ${sitterAlias} and admin support`
          }. (${successCount}/${totalCount} messages delivered)`
