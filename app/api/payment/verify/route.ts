    ...
    if (!isRecurringPayment && !bookingData.bookingId) {
      ...
      const bookingResult = await sql`
        INSERT INTO bookings (
          user_id, pet_id, service_id, date, time, duration,
          ...
          ${bookingPayload.userId}, ${bookingPayload.petId}, ${bookingPayload.serviceId},
          ${bookingPayload.date}, ${bookingPayload.time}, ${bookingPayload.duration},
    ...
    if (!isRecurringPayment && bookingData.recurring && !bookingData.recurringBookingId) {
      ...
      for (const session of sessions) {
        ...
        const recurringBookingResult = await sql`
          ...
          RETURNING *
        `;
        const recurringBooking = recurringBookingResult.rows[0];
