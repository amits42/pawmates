  ...
  useEffect(() => {
    ...
    if (searchParams && !paramsProcessed.current) {
      ...
      if (payExisting && bookingId) {
        ...
        // Get recurring session info
        const sessions = Number.parseInt(searchParams.get("sessions") || "1")
        const totalPriceFromParams = Number.parseFloat(searchParams.get("totalPrice") || "0")
        ...
        if (recurring && totalPriceFromParams > 0) {
          setServicePrice(totalPriceFromParams)
          console.log("💰 Recurring booking total price:", totalPriceFromParams)
        } else if (service) {
      ...
      if (payRecurring && recurringBookingIdParam) {
        ...
        const sessionData = {
          ...
          sequenceNumber: Number.parseInt(searchParams.get("sequenceNumber") || "1"),
        }

        setSessionDetails(sessionData)
        setServicePrice(sessionData.amount)
        setServiceName(sessionData.serviceName)

        console.log("📋 Recurring session details:", sessionData)
      }
                  ...
                  <div className="flex justify-between p-2 bg-white/50 rounded-lg">
                    ...
                    <span>
                      {sessionDetails.sessionDate
                        ? format(new Date(sessionDetails.sessionDate), "PPP")
                        : "Not scheduled"}
                    </span>
                  </div>
                  <div className="flex justify-between p-2 bg-white/50 rounded-lg">
            ...
            <CardFooter>
              ...
              >
                {loading ? (
                  ...
                  `💳 Pay ₹${servicePrice} for Session #${sessionDetails?.sequenceNumber} ✨`
                ) : isExistingBooking ? (
