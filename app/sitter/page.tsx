  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Welcome back, {sitterProfile.name}!</h1>
          <p className="text-gray-600">Here's your sitter dashboard overview</p>
        </div>
        <div className="flex items-center space-x-4">
          <Badge variant={sitterProfile.isVerified ? "default" : "secondary"} className="bg-green-100 text-green-800">
          ...
          <Avatar className="h-12 w-12">
            ...
            <AvatarFallback>{sitterProfile.name.charAt(0)}</AvatarFallback>
          </Avatar>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">From {stats.completedBookings} bookings</p>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <CardContent className="space-y-4">
            <div className="text-center">
              <Avatar className="h-20 w-20 mx-auto mb-4">
                <AvatarImage src={sitterProfile.photo || "/placeholder.svg"} alt={sitterProfile.name} />
                <AvatarFallback className="text-lg">{sitterProfile.name.charAt(0)}</AvatarFallback>
              </Avatar>
              <h3 className="font-semibold text-lg">{sitterProfile.name}</h3>
              <p className="text-sm text-gray-600">{sitterProfile.experience} experience</p>
            </div>
            ...
            <div className="pt-4">
              <Button onClick={() => router.push("/sitter/profile")} className="w-full" variant="outline">
                Edit Profile
              </Button>
            </div>
          </CardContent>
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Your latest pet care appointments</CardDescription>
          </CardHeader>
          <CardContent>
            {recentBookings.length > 0 ? (
              <div className="space-y-4">
                ...
                <Button onClick={() => router.push("/sitter/bookings")} variant="outline" className="w-full">
                  View All Bookings
                </Button>
              </div>
            ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Button onClick={() => router.push("/sitter/wallet")} className="h-16 bg-green-600 hover:bg-green-700">
          <div className="text-center">
            <DollarSign className="h-6 w-6 mx-auto mb-1" />
            <span>View Wallet</span>
          </div>
        </Button>
        <Button onClick={() => router.push("/sitter/bookings")} variant="outline" className="h-16">
          <div className="text-center">
            <Calendar className="h-6 w-6 mx-auto mb-1" />
            <span>Manage Bookings</span>
          </div>
        </Button>
        <Button onClick={() => router.push("/sitter/profile")} variant="outline" className="h-16">
          <div className="text-center">
