...
export const handleChatWithSitter = async (phoneNumber: string, petSitterPhoneNumber?: string) => {
  if (!petSitterPhoneNumber) {
    toast.error("Sitter phone number is not available.")
    return
  }
  ...
  try {
    window.open(whatsappURL, "_blank")
    toast({
      title: "Sitter Chat Started! 🎉",
      description: "Check your WhatsApp for setup messages. This creates a 3-way chat with your sitter.",
    })
  } catch (error) {
    console.error("Error opening WhatsApp:", error)
    toast.error("Could not open WhatsApp. Please try again.")
  ...
  return (
    <>
      ...
      {petSitterPhoneNumber && (
        <button onClick={() => handleChatWithSitter(phoneNumber, petSitterPhoneNumber)}>Chat with Sitter</button>
      )}
    </>
  )
}
