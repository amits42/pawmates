...
export function AuthProvider({ children }: { children: React.ReactNode }) {
      ...
      try {
        ...
        if (!res.ok || !result.valid) {
          ...
          if (storedUser) {
            ...
            console.warn("⚠️ AuthProvider: Token valid but no user/sitter found - clearing token")
            removeStoredToken()
          }
        }
      } catch (err) {
      ...
      if (data.token) {
        ...
        if (userType === "sitter") {
          ...
          if (data.sitter) {
            ...
            console.error("❌ AuthProvider: No sitter data in response")
  ...
  const updateSitterProfile = async (sitterData: Partial<Sitter>): Promise<void> => {
    if (!sitter) {
      console.error("❌ AuthProvider: Cannot update sitter profile - no sitter")
      return
    }
